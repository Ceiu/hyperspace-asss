...
struct ArenaScores
{
	...
	int keep_score;					// Whether or not the game is a server-scored game.
...
EXPORT const char info_cl_pbgoals[] = "cl_pbgoals v1.0 - Ceiu";

EXPORT int MM_cl_pbgoals(int action, Imodman *mm_, Arena *arena)
{
	if (action == MM_LOAD)
	{
		mm = mm_;
...
void LoadConfig(Arena *arena) {
	...
	if(scores) {
		...
		scores->mode = goalmode_val(cfg->GetStr(arena->cfg, "Soccer", "Mode"), GOAL_ALL);
		
		/* cfghelp: Soccer:KeepScore, arena, int, def: 1
		 * Specifies whether or not the server should keep score for this soccer game. */
		scores->keep_score = cfg->GetInt(arena->cfg, "Soccer", "KeepScore", 1);
...
void MyGoal(Arena *arena, Player *p, int bid, int x, int y)
{
	...
	if (scores->mode && scores->keep_score)
	{
		ScoreMsg(arena, NULL);
...
{
	chat->SendArenaMessage(p->arena, "Resetting game. -%s", p->name);
	ResetGame(p->arena);
}
